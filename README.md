...
## Configure me (before launch)

In `assets/app.js`, top of file:

```js
SUPABASE_URL: "TBD",       // from Supabase → Project Settings → API
SUPABASE_ANON_KEY: "TBD",  // public anon key
BACKEND_MODE: "supabase",  // or "netlify-forms"
```

Until these are set, the site works fully — logging is silently skipped (the UI never blocks on analytics).
...
**Reading data:** Supabase console → Table Editor → CSV export (weekly run charts in Excel). The public app never reads data back.
